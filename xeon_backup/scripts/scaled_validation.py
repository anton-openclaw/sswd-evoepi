#!/usr/bin/env python3
"""Scaled-up 5-node validation with K=50,000 per node.

Generates report + figures for Willem.
"""
...
print(f"=== Scaled Validation: K={K:,}, {YEARS}yr, disease yr {DISEASE_YEAR}, seed={SEED} ===")
...
print(f"{'Node':<14} {'Init':>7} {'Min':>7} {'Final':>7} {'Crash%':>7} {'Deaths':>7} {'Recov':>6} {'Δr':>8} {'Δt':>8} {'Δc':>8}")
print("-" * 100)
...
for i in range(5):
    ...
    deaths = int(res.yearly_disease_deaths[i].sum()) if hasattr(res, 'yearly_disease_deaths') else 0
    recov = int(res.yearly_recoveries[i].sum()) if hasattr(res, 'yearly_recoveries') else 0
    ...
    print(f"{names[i]:<14} {pop_init:>7,} {pop_min:>7,} {pop_final:>7,} {crash:>6.1f}% {deaths:>7,} {recov:>6} {dr:>+7.4f} {dt:>+7.4f} {dc:>+7.4f}")
...
print(f"Total: {total_init:,} → {total_final:,} ({100*(1-total_final/total_init):.1f}% decline), {total_deaths:,} disease deaths, {total_recov} recoveries")

# ─── Generate figures ─────────────────────────────────────────────────

outdir = "results/scaled_validation"
...
try:
    ...
    for i in range(5):
        ax.plot(years, res.yearly_pop[i], color=colors[i], linewidth=2, label=names[i])
    ax.axvline(DISEASE_YEAR, color='red', linestyle='--', alpha=0.5, label='Epidemic onset')
    ax.set_xlabel('Year', fontsize=12)
    ax.set_ylabel('Population', fontsize=12)
    ax.set_title(f'Population Trajectories (K={K:,}, 3-trait architecture)', fontsize=14)
    ...
    plt.close(fig)
    print(f"Saved: {outdir}/population_trajectories.png")
    ...
    for ax, (data, title, ylabel) in zip(axes, trait_data):
        for i in range(5):
            ax.plot(years, data[i], color=colors[i], linewidth=2, label=names[i])
    ...
    # Figure 3: Trait shift comparison (grouped bar)
    ...
    ax.set_ylabel('Trait shift (post-epidemic - pre-epidemic)', fontsize=11)
