...
Recovery   & $c_i$ & $n_C = 17$ & $34$--$50$ & Pathogen clearance: daily probability of $\text{I}_1$/$\text{I}_2 \to \text{R}$ transition \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item \textbf{Recovery} ($c_i$) acts \emph{during late infection}
    ...
    Recovering hosts actively clear the pathogen and transition to an
    immune state (R), removing a shedding host from the population.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Recovery} determines the daily clearance probability:
    ...
    agent to the R (recovered, immune) compartment.
\end{enumerate}
