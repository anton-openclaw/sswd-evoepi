#!/usr/bin/env python3
"""5-node validation with K=5000 per node.

Confirms recovery recording fix + rho_rec=0.05, target_mean_c=0.02.
"""
...
print(f"=== 5K Validation: K={K:,}, {YEARS}yr, disease yr {DISEASE_YEAR}, seed={SEED} ===")
...
outdir = "results/validation_5k_fixed"
...
try:
    ...
    # Need to add year 0 pop for plotting (yearly_pop is n_years, not n_years+1)
    # Actually yearly_pop shape is (N, n_years) - years 0..19
    ...
    for ax, (data, title, ylabel) in zip(axes, trait_data):
        ...
        ax.grid(True, alpha=0.3)
    fig.suptitle(f'Three-Trait Evolution (K={K:,}, ρ_rec=0.05, mean_c=0.02)', fontsize=14, y=1.02)
    fig.tight_layout()
    ...
    # Figure 4: Trait shift comparison (grouped bar)
