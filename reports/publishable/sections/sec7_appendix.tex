...
The model was calibrated against regional recovery fractions estimated from available survey data and expert assessment (Table~\ref{tab:targets}). RMSE was computed on log-transformed fractions to weight relative errors equally across the four-order-of-magnitude range of targets.
...
\begin{tabular}{lrl}
\toprule
\textbf{Region} & \textbf{Target (\%)} & \textbf{Basis} \\
\midrule
AK-PWS (Prince William Sound) & 50 & Substantial survey-documented recovery \\
AK-FN (Fairweather North) & 50 & Cold-water remote populations \\
AK-FS (Fairweather South) & 20 & Partial recovery; warmer than PWS \\
BC-N (British Columbia North) & 20 & Moderate fjord-coast recovery \\
SS-S (Salish Sea South) & 5 & Limited recovery in semi-enclosed waters \\
JDF (Juan de Fuca Strait) & 2 & Near-total loss \\
OR (Oregon) & 0.25 & Severe decline, minimal recovery \\
CA-N (California North) & 0.1 & Near-extirpation \\
\bottomrule
\end{tabular}
\end{table}
