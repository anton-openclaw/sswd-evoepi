...
\subsection{The Oregon opportunity}

Oregon emerges as the most promising region for natural recovery in the
model, retaining 17.6\% of pre-disease carrying capacity under the
baseline scenario (F01)---the highest of any region coast-wide.  Three
...
Under high-end warming (SSP5-8.5, F04), Oregon's advantage diminishes
(recovery drops to 10.4\%) because warmer summers extend the pathogen
activity window.  This underscores that Oregon's favorable position is
thermally mediated and climate-contingent.  If conservation managers are
seeking sites where natural recovery is most advanced and monitoring
is most likely to detect positive trends, Oregon merits particular
attention.

\subsection{Alaska's conditional optimism}

Alaska presents a more complicated picture.  The model predicts only
2--7\% recovery for Alaskan regions under both climate scenarios, but
...
\begin{itemize}
  \item \textbf{Alaska underprediction.}  As discussed, the model
    produces 2--7\% Alaskan recovery against ${\sim}$50\% field
