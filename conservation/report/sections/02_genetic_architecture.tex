...
Before deriving population-level predictions, we must formalize the 
individual-level genetics. Everything downstream depends on getting 
this right.

\subsection{Locus structure}

The model implements 51 biallelic loci, motivated by the 
\citet{schiebelhut2018} genome scan which identified 51 loci showing 
significant allele frequency shifts between pre- and post-SSWD 
\pyc{} populations. These are partitioned into three non-overlapping 
trait blocks:
...
\end{equation}
so $\sum_{\ell=1}^{n} \alpha_\ell = 1$ exactly. Effect sizes are 
sorted descending: $\alpha_1 \geq \alpha_2 \geq \cdots \geq \alpha_n$.

This creates a realistic effect-size distribution where a few loci 
contribute disproportionately — consistent with the observation that 
complex trait architectures typically show a roughly exponential 
distribution of effect sizes \citep{barton2002understanding}.
...
This is a \textbf{strictly additive} model: no dominance, no epistasis. 
The trait value is the weighted sum of allelic dosages. This means:
\begin{itemize}
    \item Trait values are continuous on $[0, 1]$
    \item The distribution is determined entirely by allele frequencies 
    and effect sizes
    \item Heterozygotes are exactly intermediate (no dominance deviation)
    \item Traits from different blocks are genetically independent 
    (no pleiotropy, no linkage)
\end{itemize}
...
\end{equation}
The daily infection probability is $p_{\text{inf}} = 1 - e^{-\lambda_i}$.
At $r_i = 1$, $\lambda_i = 0$: complete immunity.
At $r_i = 0$, full susceptibility.

\note{Resistance is multiplicative in the hazard rate. This means 
the fitness benefit of each unit increase in $r$ depends on 
pathogen pressure. In low-pathogen environments, the marginal 
value of resistance is small. In high-pathogen environments, 
resistance is strongly selected.}

\subsubsection{Tolerance ($t_i$)}
Damage limitation. Extends survival time in the I$_2$ (symptomatic 
terminal) stage by reducing the I$_2 \to$ D transition rate:
\begin{equation}
    ...
    \label{eq:tolerance}
\end{equation}
where $\tau_{\max} = 0.85$. At maximum tolerance ($t_i = 1$), 
the I$_2$ stage is extended by a factor of 
$1/(1 - \tau_{\max}) = 6.67$. The 5\% floor prevents immortality.
...
\end{equation}
with $a = 2$, $b = 8$ (right-skewed, most loci at low frequency). 
The scaling ensures $\E[\tau_i] \approx \bar{\tau}_{\text{target}}$.

The clip to $[0.001, 0.5]$ is important: it ensures no locus is 
fixed (always some variation to select on) and no locus has the 
protective allele at majority frequency (consistent with the 
assumption that SSWD-protective alleles were rare pre-epidemic).
